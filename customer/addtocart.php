        ...
        $nomor = date("YmdHis");
				...
				if ($stock == 0) {
		          echo "<script>alert('Stock Habis, silahkan pilih produk lain!'); window.location = 'javascript:history.back()'</script>";
						...
						$insert = mysqli_query($koneksi, "INSERT INTO cart(id_cart, tanggal, kode, nama, ukuran, harga, qty, jumlah, session)
						                                  VALUES('$nomor', '$tanggal', '$kode', '$nama', '$size', '$harga', '$qty', '$jumlah', '$session');") or die(mysqli_error());
